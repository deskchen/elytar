...
def generate_version():
    ...
    if git_tag in ["", "nightly"]:
        ...
        version = base_version + ".dev" + build_datetime_short
...
class CMakeBuild(build_ext):
    ...
    def copy_assets(self, ext):
        ...
        local_build_marker = os.environ.get("SAPIEN_LOCAL_BUILD_MARKER", "")
        local_physx_version = os.environ.get("SAPIEN_LOCAL_PHYSX_VERSION", "")
        with open(os.path.join(self.build_lib, "sapien", "version.py"), "w") as f:
            f.write('__version__="{}"\n'.format(version))
            f.write("__local_build_marker__={!r}\n".format(local_build_marker))
            f.write("__local_physx_version__={!r}\n".format(local_physx_version))
