"""Capybara DSL port of integrationTGS.cu — integrateCoreParallelLaunchTGS.

Ports the TGS rigid-body velocity integration and pose update kernel.
...
# ---------------------------------------------------------------------------
# integrateCoreParallelLaunchTGS kernel
# ---------------------------------------------------------------------------
@cp.kernel
def integrateCoreParallelLaunchTGS(
    ...
    with cp.Kernel(grid_x, threads=BLOCK_SIZE) as (bx, block):
        for tid, thread in block.threads():
            ...
            if a < num_solver_bodies:
                ...
                # ---- Read body2World from out_body2world (TGS: previous output) ----
                b2w_qx = out_body2world[a, 0] + cp.float32(0.0)
                b2w_qy = out_body2world[a, 1] + cp.float32(0.0)
                b2w_qz = out_body2world[a, 2] + cp.float32(0.0)
                b2w_qw = out_body2world[a, 3] + cp.float32(0.0)
                b2w_px = out_body2world[a, 4] + cp.float32(0.0)
                b2w_py = out_body2world[a, 5] + cp.float32(0.0)
                b2w_pz = out_body2world[a, 6] + cp.float32(0.0)
                ...
                # ---- Read solver body velocity (TGS: packed v0/v1 unpacking) ----
                readIndex = accumulated_delta_v_offset + a
                v0_x = solver_body_vel_pool[readIndex, 0] + cp.float32(0.0)
                v0_y = solver_body_vel_pool[readIndex, 1] + cp.float32(0.0)
                v0_z = solver_body_vel_pool[readIndex, 2] + cp.float32(0.0)
                v0_w = solver_body_vel_pool[readIndex, 3] + cp.float32(0.0)
                v1_x = solver_body_vel_pool[readIndex + num_solver_bodies, 0] + cp.float32(0.0)
                v1_y = solver_body_vel_pool[readIndex + num_solver_bodies, 1] + cp.float32(0.0)
                lin_vel_x = v0_x
                lin_vel_y = v0_y
                lin_vel_z = v0_z
                ang_vel_x = v0_w
                ...
                # ============================================================
                # integrateCoreTGS() — begin
                # ============================================================

                # Construct body velocities from solver output (TGS: no + initial)
                # bodyLinearVelocity = solverBodyLinVel
                body_lin_x = lin_vel_x
                body_lin_y = lin_vel_y
                body_lin_z = lin_vel_z

                # bodyAngularVelocity = sqrtInvInertia * solverBodyAngVel (no + initial)
                ...
                # Update solver body velocities with lock flags (TGS: direct, not delta)
                lin_vel_x = body_lin_x
                lin_vel_y = body_lin_y
                lin_vel_z = body_lin_z
                # Angular: lock directly
                if lock_flags & cp.int32(eLOCK_ANGULAR_X):
                ...
                # Compute motion velocities for sleep checking (TGS: no + initial)
                # linearMotionVel = motionLinVel
                lm_x = mot_lin_x
                lm_y = mot_lin_y
                lm_z = mot_lin_z
                # angularMotionVel = sqrtInvInertia * motionAngVel (no + initial)
                am_x = m00 * mot_ang_x + m01 * mot_ang_y + m02 * mot_ang_z
                am_y = m10 * mot_ang_x + m11 * mot_ang_y + m12 * mot_ang_z
                am_z = m20 * mot_ang_x + m21 * mot_ang_y + m22 * mot_ang_z
                ...
                # TGS: uses deltaBody2World from TxIData instead of velocity*dt
                # ============================================================
                if freeze == cp.int32(0):
                    # Read deltaBody2World from TxIData
                    delta_qx = solver_txidata[a, TXI_DELTA_QX] + cp.float32(0.0)
                    delta_qy = solver_txidata[a, TXI_DELTA_QY] + cp.float32(0.0)
                    delta_qz = solver_txidata[a, TXI_DELTA_QZ] + cp.float32(0.0)
                    delta_qw = solver_txidata[a, TXI_DELTA_QW] + cp.float32(0.0)
                    delta_px = solver_txidata[a, TXI_DELTA_PX] + cp.float32(0.0)
                    delta_py = solver_txidata[a, TXI_DELTA_PY] + cp.float32(0.0)
                    delta_pz = solver_txidata[a, TXI_DELTA_PZ] + cp.float32(0.0)

                    # Position: body2World.p += deltaBody2World.p
                    b2w_px = b2w_px + delta_px
                    b2w_py = b2w_py + delta_py
                    b2w_pz = b2w_pz + delta_pz

                    # Rotation: body2World.q = normalize(deltaBody2World.q * body2World.q)
                    # Quaternion multiply: delta_q * b2w_q
                    # (a,b,c,d)*(e,f,g,h) where d,h are scalar (w) parts:
                    # scalar: d*h - (a*e + b*f + c*g)
                    # vector: d*(e,f,g) + h*(a,b,c) + (b*g-c*f, c*e-a*g, a*f-b*e)
                    res_x = delta_qw * b2w_qx + b2w_qw * delta_qx + (delta_qy * b2w_qz - delta_qz * b2w_qy)
                    res_y = delta_qw * b2w_qy + b2w_qw * delta_qy + (delta_qz * b2w_qx - delta_qx * b2w_qz)
                    res_z = delta_qw * b2w_qz + b2w_qw * delta_qz + (delta_qx * b2w_qy - delta_qy * b2w_qx)
                    res_w = delta_qw * b2w_qw - (delta_qx * b2w_qx + delta_qy * b2w_qy + delta_qz * b2w_qz)
                ...
                # ---- Write output solver velocity (TGS: direct lin_vel) ----
